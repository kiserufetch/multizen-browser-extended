...
- [x] **T1 — Add `electron-updater` dependency.**
...
- [x] **T5 — Wire service + IPC in `index.ts`.**
...
- [x] **T10 — Settings "Updates" section.**
...
- [~] **T13 — Independent review loop, then merge.**
  ...
  _Depends:_ T12. _Status:_ review loop DONE — round 1 found 5 issues (all fixed),
  round 2 returned clean (independent sign-off). **Merge to `master` awaiting
  user go-ahead** (release/tag also pending confirmation).
